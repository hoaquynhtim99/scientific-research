...
function get_agencies_alias(id) {
    var title = strip_tags(document.getElementById('idtitle').value);
    if (title != '') {
        $.post(
            script_name + '?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=agencies&nocache=' + new Date().getTime(),
            'changealias=1&title=' + encodeURIComponent(title) + '&id=' + id, function(res) {
            if (res != "") {
                document.getElementById('idalias').value = res;
            } else {
...
function nv_change_agencies_weight(id) {
...
function nv_change_agencies_status(id) {
...
function nv_delele_agencies(id) {
    if (confirm(nv_is_del_confirm[0])) {
        $.post(
            script_name + '?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=agencies&nocache=' + new Date().getTime(),
            'delete=1&id=' + id, function(res) {
        ...
        if (action == 'delete') {
            if (confirm(nv_is_del_confirm[0])) {
                $.post(
                    ...
                    'delete=1&listid=' + listid, function(res) {
                    var r_split = res.split("_");
                    if (r_split[0] == 'OK') {
                        location.reload();
                    } else {
